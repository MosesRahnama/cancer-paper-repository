...
To ground the framework's predictions in existing data, we performed a systematic correlation, subtype, and survival analysis using bulk RNA-seq expression from The Cancer Genome Atlas (TCGA), accessed via the ISB Cancer Genomics Cloud BigQuery tables~\cite{isb2023cgc}. We analyzed six cohorts spanning $n=3{,}611$ tumor samples: TCGA-SKCM (melanoma, $n=472$), TCGA-LUAD (lung adenocarcinoma, $n=530$), TCGA-BRCA (breast, $n=1{,}113$), TCGA-COAD (colon, $n=473$), TCGA-HNSC (head and neck squamous cell carcinoma, $n=522$), and TCGA-LUSC (lung squamous cell carcinoma, $n=501$). We extracted transcripts-per-million (TPM) values for 21 target genes spanning immune checkpoint (CD274/PD-L1, PDCD1LG2, PDCD1), MHC class~I (HLA-A, HLA-B, HLA-C, B2M), gap junction (GJA1/Cx43, GJB2/Cx26, GJA5/Cx40, GJB6/Cx30), circadian clock (ARNTL/BMAL1, CLOCK, PER1, PER2, CRY1, CRY2), and differentiation markers (CDH1, VIM, MYC, TP53). Correlation analyses were performed at tumor-sample level (Spearman $\rho$ on log$_2$(TPM$+$1), with FDR correction). Survival analyses were performed at patient level by retaining one tumor sample per case before clinical merge ($n=3{,}089$ valid OS cases), and we report both raw log-rank $p$ values and Benjamini--Hochberg adjusted $q$ values. For robustness in SKCM and LUAD, we also fit multivariable Cox models with clinical covariates (age, sex, stage as categorical indicators), tested a continuous PD-L1 $\times$ clock interaction term, and applied BH correction across pre-specified primary robustness tests.
...
This means that tumors with higher PD-L1 expression tend to show \emph{lower} circadian CV (more coherent clock expression) across all cancer types tested. This initially appears to contradict the prediction that checkpoint-engaged tumors have degraded temporal coordination. However, this pattern is consistent with an \emph{immune-hot} tumor phenotype: interferon-$\gamma$ signaling from infiltrating T cells can co-activate PD-L1 and other immune-responsive transcriptional programs~\cite{pd1pdl1review,pd1resistance2018}, producing an inflamed but checkpoint-protected microenvironment. The boundary failure in these tumors is not temporal decoherence but \emph{active masking}---PD-L1 upregulation as an adaptive evasion response in tumors that retain enough signaling infrastructure to respond to immune pressure. Because these inferences are from bulk RNA-seq, this mechanism remains a plausible interpretation rather than direct causal proof. In sensitivity analyses controlling for purity and microenvironment covariates, the circadian CV--PD-L1 association remained negative in both discovery cohorts (SKCM partial $r=-0.238$, $p=4.29\times10^{-5}$; LUAD partial $r=-0.144$, $p=0.00658$; Section~\ref{subsec:robustness}). The replication across six cancer types spanning distinct tissue origins (epithelial, mesenchymal, squamous) strongly supports the generalizability of this finding.
...
If the boundary-failure mode carries biological significance beyond gene-expression patterns, it should associate with clinical outcomes. We compared overall survival between Active Masking and Decoherence tumors using Kaplan--Meier analysis with log-rank tests on a patient-level deduplicated cohort (one tumor sample per case).
...
To reduce confounding risk from clinical composition, we re-estimated the survival signal in SKCM and LUAD using multivariable Cox models in the deduplicated patient-level cohort (one tumor sample per case). Covariates included boundary mode, age, sex, and stage as categorical indicators (II/III/IV plus stage-missing; stage I reference), with optional microenvironment covariates from Thorsson et~al.\ data when available (purity, lymphoid/myeloid/leukocyte infiltration proxies, stromal fraction, IFN-$\gamma$ response, TGF-$\beta$ response). We also tested a continuous interaction term between PD-L1 and an orthogonal clock metric (circadian CV computed from CLOCK, PER2, CRY1, CRY2; excluding ARNTL/PER1 to reduce definitional coupling), and a sensitivity model adding continuous PD-L1 and B2M terms to test whether boundary mode adds signal beyond these components. Primary robustness tests were BH-corrected across cohorts/tests.
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item \textbf{Partial-correlation controls:} The circadian CV--PD-L1 association remained negative after adjusting for available control covariates (SKCM partial $r=-0.238$, $p=4.29\times10^{-5}$; LUAD partial $r=-0.144$, $p=0.00658$). Using the orthogonal clock metric, this adjusted association was strong in LUAD (partial $r=-0.269$, $p=2.52\times10^{-7}$) and weaker/non-significant in SKCM ($p=0.105$), indicating cohort-dependent robustness.
\end{itemize}

These robustness analyses remain observational and complete-case (covariate availability reduced model sample sizes: e.g., AM-vs-DC models used $n=175$ in SKCM and $n=183$ in LUAD). Time-interaction proportional-hazards screens indicated widespread potential non-proportionality across covariates, so Cox coefficients should be interpreted as time-averaged associations pending stratified or explicitly time-varying follow-up models.
...
Circadian CV showed \textbf{no FDR-significant correlation} with tumor stage in any of the six cancer types (all circadian-stage tests $q > 0.05$). This is an important negative result: it is consistent with an early-established, persistent circadian restructuring pattern rather than a strictly late-stage accumulation effect. However, absence of stage association in this dataset does not by itself prove temporal ordering or causality. (Other non-circadian markers, such as CD274/B2M and gap-junction summaries in specific cohorts, showed modest stage associations after global FDR correction.)

This finding is consistent with the framework's prediction that boundary failure can be a foundational event---a shift in the cell's relationship with the organism---rather than only a downstream consequence of tumor burden. It motivates, but does not yet establish, the use of circadian-based biomarkers for early disease settings.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{cancer-paper-repository/results/stage_circadian_cv_boxplot.png}
\caption{Circadian CV by pathologic tumor stage across six TCGA cancer types. No significant association between circadian CV and stage was observed in any cancer type (all FDR $> 0.05$), consistent with a stage-stable circadian restructuring pattern.}
\label{fig:stage}
\end{figure}
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item \textbf{Circadian restructuring is stage-stable in this dataset.} No FDR-significant association between circadian CV and tumor stage was observed, which is consistent with early establishment and persistence but does not alone establish temporal causality.
