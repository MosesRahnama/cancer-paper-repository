...
We defined a per-sample circadian coherence proxy as the coefficient of variation (CV) across the six core clock genes (ARNTL, CLOCK, PER1, PER2, CRY1, CRY2), where lower CV indicates more uniform (coherent) clock-gene expression. The initial Hypothesis~1 expectation was that checkpoint engagement would track reduced coherence; the TCGA data instead support a refined two-mode structure in which checkpoint-high tumors can show clock restructuring/locking (Active Masking) rather than simple temporal decoherence.
...
To reduce confounding risk from clinical composition, we re-estimated the survival signal in SKCM and LUAD using multivariable Cox models in the deduplicated patient-level cohort (one tumor sample per case). Covariates included boundary mode, age, sex, and stage as categorical indicators (II/III/IV plus stage-missing; stage I reference). We also tested a continuous interaction term between PD-L1 and an orthogonal clock metric (circadian CV computed from CLOCK, PER2, CRY1, CRY2; excluding ARNTL/PER1 to reduce definitional coupling). Primary robustness tests were BH-corrected across cohorts/tests.

\begin{itemize}[leftmargin=1.5em]
    \item \textbf{Adjusted boundary-mode effect:} In SKCM, Active Masking remained associated with lower hazard than Decoherence after covariate adjustment (HR $=0.555$, $p=0.00194$, BH $q=0.0116$). In LUAD, the direction was similar but non-significant (HR $=0.616$, $p=0.110$, $q=0.151$).
    \item \textbf{PD-L1 $\times$ clock interaction:} In SKCM, the continuous interaction term was significant (HR $=0.209$, $p=0.00504$, BH $q=0.0151$), supporting non-additive coupling between immune-checkpoint engagement and clock-state structure. In LUAD, the interaction was not significant ($p=0.638$).
    \item \textbf{Within high PD-L1 tumors:} The orthogonal clock effect was nominal in SKCM ($p=0.075$) and non-significant in LUAD ($p=0.126$), indicating that the added clock contribution is strongest as an interaction signal in SKCM rather than a uniform marginal effect.
\end{itemize}

These robustness analyses remain observational. Time-interaction proportional-hazards screens indicated potential non-proportionality for several covariates, so Cox coefficients should be interpreted as time-averaged associations pending stratified or explicitly time-varying follow-up models. Purity and immune-infiltration covariates were pre-specified in the analysis pipeline but were not available in the current merged TCGA table; those controls remain a prioritized next-step sensitivity analysis.
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item \textbf{Boundary-failure mode is associated with survival heterogeneity.} Active Masking tumors show better overall survival than Decoherence tumors in melanoma (significant after BH correction: $p = 0.0011$, $q = 0.0066$), with a directionally similar but nominal result in lung adenocarcinoma ($p = 0.049$, $q = 0.147$). In clinical-adjusted Cox robustness models (SKCM/LUAD), the boundary-mode signal remains significant in SKCM (HR $=0.555$, $q = 0.0116$) but not in LUAD.
