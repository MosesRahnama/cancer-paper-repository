...
The framework is not purely theoretical. Analysis of $n=3{,}611$ tumor samples across six TCGA cohorts, plus a deduplicated patient-level survival cohort ($n=3{,}089$ with valid overall-survival data), shows that boundary-failure mode is clinically associated with outcome. Figure~\ref{fig:survival-preview} summarizes FDR-adjusted survival evidence across all cohorts: Active Masking vs.\ Decoherence is significant in melanoma (log-rank $p = 0.0011$, BH $q = 0.0066$), directionally similar but nominal in lung adenocarcinoma ($p = 0.049$, $q = 0.147$), and quartile-based circadian CV alone is not significant in any cohort. Full Kaplan-Meier curves are presented in Section~\ref{sec:tcga}.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{results/survival_test_qvalue_summary.png}
\caption{\textbf{Cross-cohort survival evidence summary (FDR-adjusted).} Grouped bars show $-\log_{10}(q)$ across six TCGA cohorts for two pre-specified survival tests: Active Masking vs.\ Decoherence and circadian-CV quartile (Q1 vs Q4). The AM-vs-DC contrast shows strongest support in melanoma (SKCM), while circadian quartiles alone are non-significant across cohorts. Asterisks denote FDR-significant tests ($q<0.05$).}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{experiments/tcga/figures/control_budget_combined.png}
...
\caption{Hazard-ratio forest plot for primary robustness models in SKCM and LUAD. Points show Cox HR estimates with 95\% confidence intervals; green markers denote FDR-significant tests. AM-vs-DC remains protective in adjusted models (stronger in LUAD), and PD-L1$\times$clock interaction is significant in SKCM.}
...
\caption{Spearman stage-trend effect sizes for circadian CV across all six cohorts. Most effects are near zero after BH correction within circadian stage-trend tests, reinforcing a largely stage-stable circadian signal in this dataset.}
